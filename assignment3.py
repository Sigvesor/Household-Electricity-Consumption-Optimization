...
class Optimisation:
    ...
    def optimise(self):
        ...
        # c = array(k)
        power_matrix = self.time_matrix.to_numpy() * self.apps['p'].to_numpy()
        price_matrix = power_matrix.T * self.pricing
        A_ub = power_matrix
        b_ub = np.ones((self.nr_of_hours, 1)) * 20
        A_eq = power_matrix.transpose()
        b_eq = self.apps['e'].to_numpy()
        # A_eq = app_matrix * self.apps['p'].to_numpy()
        # b_eq = self.apps['e'].to_numpy().transpose()
        c = price_matrix.reshape(self.time_matrix.size)
...
class Prob2(Optimisation):
    def __init__(self, data_name='Krsand'):
        ...
        self.pricing = pd.read_csv('nordpool_20200303.csv', decimal=',')[data_name][0:self.nr_of_hours].to_numpy() / 1000 # €/kWh
        self.rtp_noise()
        self.optimise()
...
class Prob3(Optimisation):
    def __init__(self, data_name='Krsand'):
        ...
        self.pricing = pd.read_csv('nordpool_20200303.csv', decimal=',')[data_name][0:self.nr_of_hours] / 1000 # €/kWh
        self.optimise()
